...
Component({
  ...
  methods: {
    ...
    handleConfirm() {
      ...
      if (!formValue.winners.length) {
        wx.showToast({ title: '请先选择胡牌人', icon: 'none' })
        return
      }
      if (formValue.isZimo && formValue.winners.length > 1) {
        wx.showToast({ title: '当前不能，请先只保留一位胡牌人', icon: 'none' })
        return
      }
      if (formValue.winnerDetails.length !== formValue.winners.length) {
        wx.showToast({ title: '当前不能，请先完成胡牌配置', icon: 'none' })
        return
      }
      if (!formValue.isZimo && !formValue.loser) {
        wx.showToast({ title: '请先选择点炮人', icon: 'none' })
        return
      }
